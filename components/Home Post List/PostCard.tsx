        ...
        <Link href={`/post/${ post.slug.current }`}>
            <div className='flex  items-center w-full bg-white mb-10'>
                <div className='space-y-2 flex-1'>
                    ...
                    <h2 className='text-lg md:text-2xl font-bold cursor-pointer'>{post.title}</h2>
                    <h3 className='text-gray-600 text-sm md:text-lg'>{post.description}</h3>
                    <div className='text-gray-600 text-sm flex justify-between items-center'>
                        ...
                        <BookmarkIcon className='text-stone-800 h-6 w-6 cursor-pointer ' fill='none' />
                    </div>
                </div>
                <img className='h-24 w-24 md:h-32 lg:h-44 md:w-32 lg:w-44 object-cover ml-5' src={urlFor( post.mainImage ).url()!} alt="Blog Image" />
            </div>
        </Link>
    )
}
